 ...
 *  @param color          print color, 0=>red, 1=>black
 *
 */
- (void)cpclSetPrintColor:(NSInteger)color;
 ...
 *  @param compress     打印机支持的压缩类型
 *  @param isPackage    是否需要分包
 *  @return false 表示图片转换失败 true 转换成功
 *
 *  \~english
 *
 *  Print Image
 ...
 *  @param compress     The type of compression supported by the printer
 *  @param isPackage    Whether the pictures need to be subcontracted
 *  @return false Conversion failed    true conversion succeeded
 */
- (BOOL)cpclPrintBitmapWithXPos:(NSInteger)xPos
                           yPos:(NSInteger)yPos
                   base64String:(NSString *_Nullable)base64String
                     bitmapMode:(PTBitmapMode)bitmapMode
                       compress:(PTBitmapCompressMode)compress
                      isPackage:(BOOL)isPackage;
 ...
 *  @param fontName   字体名..eg: SIMSUN.TTF \TT0003M_.TTF
 ...
 *  \~english
 *
 *  print  Vector Fonts text
 *
 *  @brief print scaled text in CPCL from either a scaled or TrueType font.
 *  @param fontName   font name eg: SIMSUN.TTF \TT0003M_.TTF
 *  @param xScale       The X size of the scaled font, in points.
 *  @param yScale       The Y size of the scaled font, in points.
 ...
 *
 */
- (void)cpclTrueTypeText:(NSString *_Nullable)fontName xScale:(NSInteger)xScale yScale:(NSInteger)yScale x:(NSInteger)x y:(NSInteger)y text:(NSString * _Nonnull)text;
 ...
 *
 *  @brief print scaled text in CPCL from either a scaled
 *  @param scaledFont   A scaled font used to create the text.
 *  @param xScale       The X size of the scaled font, in points.
 *  @param yScale       The Y size of the scaled font, in points.
 *  @param x            The X origin of the scaled text in units.
 ...
 *  \~chinese
 *
 *  获取电量，该接口为自定义功能，部分机型才支持，<000902> 表示92%
 *
 *  @brief 获取电量1
 *
 *  \~english
 *
 *  get Printer Battery，This interface is a custom function, which is only supported by some models，<000902> is 92%
 *
 *  @brief get Printer Battery1
 *
 */
 ...
 *  \~chinese
 *
 *  获取电量，该接口为自定义功能，部分机型才支持,eg:A400，<38360000> 表示86%
 *
 *  @brief 获取电量2
 *
 *  \~english
 *
 *  get Printer Battery，This interface is a custom function, which is only supported by some models,eg:A400，,<38360000> is 86%
 *
 *  @brief get Printer Battery2
 *
 */
- (void)getPrinterBatteryLevel;

/*!
 *  \~chinese
 *
 *  设置蓝牙名称，该接口为自定义功能，部分机型才支持,eg:A400
 *  name的长度转成data后，不能超过32个字节
 *
 *  @brief 设置蓝牙名称
 *
 *  \~english
 *
 *  set Bluetooth name，This interface is a custom function, which is only supported by some models,eg:A400
 *  The length of name converted to data cannot exceed 32 bytes
 *
 *  @brief Set Bluetooth name
 *
 */
- (void)setBluetoothName:(NSString *_Nullable)name;
 ...
 *  \~english
 *
 *  Write RFID;The data length range refers to the storage area enumeration;The TID section is read-only and not writable，The length of the read can be used directly by the maximum value of the region
 *
 *  @brief Write RFID
 *
