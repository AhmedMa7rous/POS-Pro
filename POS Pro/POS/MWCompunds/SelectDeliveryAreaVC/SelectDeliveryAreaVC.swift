...
class SelectDeliveryAreaVC: UIViewController {

    
    @IBOutlet weak var selectUsersTable: UITableView!
//Missing in merge
    var dataList:[pos_delivery_area_class]?
    var selectDataList:[pos_delivery_area_class]?
    var completionBlock:(([pos_delivery_area_class])->())?
    ...
    func isItemSelect(_ item:pos_delivery_area_class)->Bool{
...
//         self.completionBlock?(selectDataList ?? [])
   ...
   static func createModule(_ sender:UIView?,selectDataList:[pos_delivery_area_class]?,dataList:[pos_delivery_area_class]) -> SelectDeliveryAreaVC {
        let vc:SelectDeliveryAreaVC = SelectDeliveryAreaVC()
...
extension SelectDeliveryAreaVC:UITableViewDelegate,UITableViewDataSource{
    func setupTable(){
