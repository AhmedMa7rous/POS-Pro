...
class sessionList_report: ReportViewController  {
    ...
    func printOrder_html() -> String {
        ...
        var html = baseClass.get_file_html(filename: "session_list",showCopyRight: true)
//        let pos = SharedManager.shared.posConfig()
    ...
    func build_report(html: String) -> String
      {
                ...
                {
                    rows_order.append(" <table  style=\"width: 100%;text-align: left;border: 4px solid black;padding: 10px;margin-top: 20px;\"> "   )
                    let session = pos_session_class(fromDictionary: arr_data[i])
                    ...
                    let dt_end = Date(strDate: session.end_session!, formate: baseClass.date_fromate_satnder,UTC: true)
                    let endDate = dt_end.toString(dateFormat: baseClass.date_fromate_satnder_12h, UTC: false)
                    
                    
                    rows_order.append("<tr> <td>  <b>ID:</b>  </td><td> \(session.id) </td> </tr>")
                    rows_order.append("<tr> <td style=\"width: 30%;\">  <b>Server ID:</b>  </td><td> \(session.server_session_id) </td></tr>")
                    rows_order.append("<tr> <td>  <b>Name:</b>  </td><td>  \(session.server_session_name) </td></tr>")
                    rows_order.append("<tr> <td>  <b>Start:</b>  </td><td>  \(startDate) </td></tr>")
                    rows_order.append("<tr> <td>  <b>End:</b>  </td><td>  \(endDate) </td></tr>")
                    rows_order.append("<tr> <td>  <b>Cash in:</b>   </td><td> \(session.start_Balance) </td></tr>")
                    rows_order.append("<tr> <td>  <b>Cash out:</b>   </td><td> \(session.end_Balance) </td></tr>")
                    rows_order.append("<tr> <td>  <b>Status:</b>  </td><td> \(session.isOpen ? "Open":"Closed") </td></tr>")

                    rows_order.append(" </table> "   )
    ...
    func get_query() -> (result:[[String : Any]],filter_session:String)
    {
        ...
        let sql = """
        SELECT  * from pos_session  WHERE id in (\(sessions)) and posID =  \(pos.id)
        """
