...
class SettingsViewController: UIViewController, CaptureHelperDevicePresenceDelegate, CaptureHelperDeviceManagerPresenceDelegate{
                ...
                if status == .enable {
                    ...
                    if status == .notSupported {
                        capture.setSoftScanStatus(.supported, withCompletionHandler: { (result) in
                         SharedManager.shared.printLog("setting softscan to supported returned \(result.rawValue)")
                        })
                    }
                }
            }
    ...
    @IBAction func changeSoftScan(_ sender: AnyObject) {
        if(!softscan.isOn){
           SharedManager.shared.printLog("disabling SoftScan...")
            CaptureHelper.sharedInstance.setSoftScanStatus(.disable, withCompletionHandler: { (result) in
             SharedManager.shared.printLog("disabling softScan returned \(result.rawValue)")
            })
        }
        else{
           SharedManager.shared.printLog("enabling SoftScan...")
    ...
    */
    func didNotifyArrivalForDevice(_ device: CaptureHelperDevice, withResult result: SKTResult) {
       SharedManager.shared.printLog("Settings: Device Arrival")
    }
    ...
    */
    func didNotifyRemovalForDevice(_ device: CaptureHelperDevice, withResult result: SKTResult) {
       SharedManager.shared.printLog("Settings: Device Removal")
    }
    
    func didNotifyArrivalForDeviceManager(_ device: CaptureHelperDeviceManager, withResult result: SKTResult) {
       SharedManager.shared.printLog("Settings: Device Manager Arrival")
    ...
    func didNotifyRemovalForDeviceManager(_ device: CaptureHelperDeviceManager, withResult result: SKTResult) {
       SharedManager.shared.printLog("Settings: Device Manager Removal")
        deviceManager = nil
    }
}
*/
