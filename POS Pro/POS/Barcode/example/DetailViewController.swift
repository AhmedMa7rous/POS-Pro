    ...
    @IBAction func onSoftScanTrigger(_ sender: AnyObject) {
        if let scanner = softScanner as CaptureHelperDevice? {
            ...
            device.setTrigger(.start, withCompletionHandler: { (result) in
               SharedManager.shared.printLog("triggering the device returns: \(result.rawValue)")
            })
        }
    }
...
//            }
    ...
    func didReceiveDecodedData(_ decodedData: SKTCaptureDecodedData?, fromDevice device: CaptureHelperDevice, withResult result:SKTResult) {
       SharedManager.shared.printLog("didReceiveDecodedData in the detail view with result: \(result.rawValue)")
        if result == .E_NOERROR {
            if let rawData = decodedData!.decodedData {
                let rawDataSize = rawData.count
               SharedManager.shared.printLog("Size: \(rawDataSize)")
               SharedManager.shared.printLog("data: \(rawData)")
                let str = decodedData!.stringFromDecodedData()
               SharedManager.shared.printLog("Decoded Data \(String(describing: str))")
                DispatchQueue.main.async {
    ...
    func didNotifyArrivalForDevice(_ device: CaptureHelperDevice, withResult result:SKTResult) {
       SharedManager.shared.printLog("didNotifyArrivalForDevice in the detail view")
