...
class MasterViewController:
 ...
 /*
        ...
        AppInfo.appID = "ios:rabeh.pos"
        AppInfo.appKey = "MC4CFQDGFWtEdXplg5HQE9NQIUDwDikZuwIVAKlap2ZRDWZSyxYrSybMZ3tJ50MO"
 */
 AppInfo.appKey="MC0CFHwC17ZFoGvNXWd97WvdA+rDVA7CAhUAhy4ACNeZzWBGtTQohCmM/qZ9PTw="
 ...
 AppInfo.developerID="e546e129-75a4-ec11-b3fe-000d3a308d89"
    ...
    func didNotifyArrivalForDevice(_ device: CaptureHelperDevice, withResult result: SKTResult) {
       SharedManager.shared.printLog("Main view device arrival:\(String(describing: device.deviceInfo.name))")
                ...
                if result == .E_NOERROR {
                    ...
                    if localAck == SKTCaptureDeviceDataAcknowledgment.on {
                        ...
                        device.setDataAcknowledgment(localAck, withCompletionHandler : {(result, propertyResult) in
                            if result != .E_NOERROR {
                               SharedManager.shared.printLog("Set Local Acknowledgment returns: \(result.rawValue)")
                            }
                        })
                    }
                }
                ...
                if result == .E_NOERROR {
                    if decodeAction != .none {
                        ...
                        device.setDecodeAction(decodeAction: decodeAction, withCompletionHandler:{(result: SKTResult) in
                            if result != .E_NOERROR {
                               SharedManager.shared.printLog("Set Decode Action returs: \(result.rawValue)")
                            }
                        })
                    }
                }
                    ...
                    if var localAck = dataAcknowledgment {
                        if localAck == .off {
                            ...
                            device.setDataAcknowledgment(localAck, withCompletionHandler: {(result: SKTResult) in
                                if result != .E_NOERROR {
                                   SharedManager.shared.printLog("Set Data Acknowledgment returns: \(result.rawValue)")
                                }
                            })
                        }
                    }
                ...
                if result == .E_NOERROR {
                    if decodeAction == .none {
                        ...
                        device.setDecodeAction(action, withCompletionHandler: { (result) in
                            if result != .E_NOERROR {
                               SharedManager.shared.printLog("Set Decode Action returns: \(result.rawValue)")
                            }
           ...
           SharedManager.shared.printLog("Decoded Data \(String(describing: string))")
                ...
                device.setDataConfirmationWithLed(led: .green, withBeep: .good, withRumble: .good, withCompletionHandler: {(result) -> Void in
                    if result != SKTCaptureErrors.E_NOERROR {
                       SharedManager.shared.printLog("Set Data Confirmation returns: \(result.rawValue)")
                    }
                })
            #endif
        }
