//
//  Fix_Discount_on_Quantity+ext.swift
//  pos
//
...
class Fix_Discount_on_Quantity_Class: NSObject {
    ...
    func check(  listPromotion:[[String:Any]]) {
        ...
        let avalible  =  helper.validate2(newlistPromotion,.Fix_Discount_on_Quantity)
        ...
        guard  avalible._avalible else {
            ...
            for condition in newlistPromotion {
                ...
                let dic = promotion_helper_class.remove_perfiex(dic: condition, prfiex: "quantity_discount_amt_")
...
//        for condition in listPromotion {
            ...
            let quantity_discount = quantity_discount_amt_class(fromDictionary: promotion_helper_class.remove_perfiex(dic: condition, prfiex: "quantity_discount_amt_"))
            let quantity =  quantity_discount.quantity_amt
            let discount_price = quantity_discount.discount_price
            let discount_one_line_price = discount_price / quantity
 
            let total_products_qty = helper.get_total_qty_for(product_id:promotion.product_id_amt)

//            if total_products_qty != quantity
//            {
//
//                continue
//            }
...
//                        continue
                ...
                else if index == 0 && total_products_qty > quantity
                {
                     break
                }
                else
...
//                    continue
            ...
            if promotion.product_id_amt != 0
            {
                
 
                
                let q_products_cannot_applied = total_products_qty.toInt()  %  quantity.toInt()
                ...
                let lst_products = order.pos_order_lines.filter({$0.product_id  == promotion.product_id_amt})
    ...
    func check_old(  listPromotion:[[String:Any]]) {
        ...
        guard  helper.validate(listPromotion,.Fix_Discount_on_Quantity) else {
        ...
        for condition in listPromotion
        {
            ...
            let quantity =  quantity_discount.quantity_amt //condition["quantity_discount_amt_quantity_amt"] as? Double ?? 0
            var discount_price = quantity_discount.discount_price //condition["quantity_discount_amt_discount_price"] as? Double ?? 0
            
            let num_applied = (line.qty / quantity).toInt()
            discount_price = discount_price * Double(num_applied)
            
            if num_applied >  quantity_discount.no_of_applied_times
            {
                break
            }
      
              if line.product_id != promotion.product_id_amt
             {
                  break
             }
             ...
             {
                add_discount_on_product(new_line: line, discount_dis_x: discount_price, pos_promotion_id: promotion.id, pos_conditions_id: quantity_discount.id)
                  delegate?.reloadTableOrders(re_calc:true,reSave: true)
    ...
    func reset_discount(get_discount:quantity_discount_amt_class,promotion:pos_promotion_class)
    {
