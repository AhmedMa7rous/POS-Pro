...
//        self.calendar = Calendar.init(identifier: .gregorian)
        ...
        if let selectedDate = selectedDate {
            let day = VADay(date: selectedDate, state: .selected, calendar: self.calendar)
            selectedDays = [day]
        }
...
//        self.calendar.timeZone = TimeZone(abbreviation: "UTC")!
        ...
        let startDate = startDate ?? self.calendar.date(byAdding: .year, value: -1, to: Date())!
        let endDate = endDate ?? self.calendar.date(byAdding: .year, value: 1, to: Date())!
